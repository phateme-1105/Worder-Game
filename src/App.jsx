...
function App() {
  ...
  useEffect(() => {
    if (wordSet.size > 0 && gameMode && isUserTurn && !isGameFinished) {
      setTimeLeft(20);
  ...
  }, [wordSet, gameMode, correctWord, isUserTurn, isGameFinished, pos.wordPos]);

  const handleTimeEnd = () => {
    const newBoard = [...board];
    newBoard[pos.wordPos] = Array(correctWord.length).fill("");
    setBoard(newBoard);
    setIsUserTurn(false);
    setTimeout(() => {
      robotGuessWord({
  ...
  const handleDelete = () => onDelete(board, pos, setBoard, setPos);
